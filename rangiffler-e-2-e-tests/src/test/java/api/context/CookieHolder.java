...
public class CookieHolder {
    ...
    public void removeCookie(String cookie) {
        getAll().get(COOKIE_KEY).removeIf(c -> c.contains(cookie));
    }

    public String getCookieByName(String cookie) {

        final List<String> strings = getAll().get(COOKIE_KEY);
        System.out.println("COOKIES :" + strings);
        return strings.stream().filter(c -> c.contains(cookie)).findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Not found cookie: " + cookie + " in cookie storage!"));
    }

    public String getCookieValue(String cookie) {
        return getCookieByName(cookie).split("=")[1];
    }
